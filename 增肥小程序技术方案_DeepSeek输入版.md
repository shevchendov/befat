...
2. **饮食记录（文字 + 拍照识菜双模式）**：文字描述走 DeepSeek 解析；拍照识菜走智谱 GLM 视觉单次多模态直接输出营养 JSON，输出食物名称+估算份量+热量+蛋白质，结构化存库
...
### 拍照识菜（已实现）✅
拍照/选图 → 前端离屏 Canvas 等比压缩（Max 800px）转 base64 → 云函数 `parseFoodLog` 按模式单次调用：

1. **图片模式（单次多模态）**：智谱 GLM 视觉模型（默认 `glm-4v-flash`）看图后**直接输出营养 JSON**（份量 + 热量 + 蛋白质），一次调用完成识菜 + 营养计算
2. **文本模式**：DeepSeek（默认 `deepseek-v4-flash`）根据文字描述输出营养 JSON

两模式模型名、API Key、Base URL 均已抽为环境变量（`VISION_*` / `NUTRITION_*`），支持多厂商无部署切换。详细设计见《拍照识菜功能设计规格书.md》。
...
3. **拍照识菜**：log-food 图片模式单次多模态（GLM 视觉直接出营养 JSON）✅
