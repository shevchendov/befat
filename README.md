...
- **附近推荐（约饭吧 / 去运动）** — 首页快捷入口按目标方向动态展示，直达附近推荐视图；增重搜餐饮、减重搜运动场所（公园/健身房/绿道/游泳馆），支持关键词/标签搜索、导航、电话，后端多服务商切换
...
│   │   ├── index/           # 首页（每日概览 + 目标进度卡 + BMI 标签 + 约饭吧/去运动入口）
│   │   ├── log-food/        # 记录饮食（文字/拍照识别 → 编辑 → 保存）
│   │   ├── weight-track/    # 体重打卡 + 趋势图（无显式入口，待清理）
│   │   ├── daily-menu/      # 每日食谱（gain）/ 燃脂锦囊（lose）+ 附近推荐视图 + 收藏抽屉
